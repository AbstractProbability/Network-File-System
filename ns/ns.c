...
SSInfo* find_unbacked_ss() {
    ...
    while (current != NULL) {
        if (current->is_active && !current->backed_up && current->partner_name[0] == '\0') {
...
// Send pairing command to a storage server
void send_pair_command(SSInfo* ss, const char* partner_name, const char* partner_ip, int partner_port, int should_send_sync) {
    pthread_mutex_lock(&ss->cmd_lock);
    
    // Send opcode
    NSOpCode opcode = NS_SS_PAIR_AND_SYNC;
    send(ss->cmd_sock, &opcode, sizeof(NSOpCode), 0);
    
    // Send partner info
    send(ss->cmd_sock, partner_name, MAX_PATH_LEN, 0);
    send(ss->cmd_sock, partner_ip, INET_ADDRSTRLEN, 0);
    send(ss->cmd_sock, &partner_port, sizeof(int), 0);
    send(ss->cmd_sock, &should_send_sync, sizeof(int), 0);
    
    pthread_mutex_unlock(&ss->cmd_lock);
    
    printf("NS: Sent pair command to SS '%s' -> partner '%s' (%s:%d), should_send_sync=%d\n",
...
void* handle_connection(void* socket_desc) {
    ...
    if (recv(sock, &init_pkt, sizeof(InitialPacket), 0) <= 0) {
    ...
    if (init_pkt.type == CONN_CLIENT) {
        ...
        if (recv(sock, &info_pkt, sizeof(SS_Info_Packet), MSG_PEEK) <= 0) {
    ...
    while (1) {
        ...
        while (current != NULL) {
            if (current->is_active) {
                ...
                if (send(current->cmd_sock, &heartbeat_op, sizeof(NSOpCode), 0) <= 0) {
