...
void* handle_client_session(void* arg) {
    ...
    ClientRequest req;
    while (recv(sock, &req, sizeof(ClientRequest), 0) > 0) {
        
        // --- Log the operation ---
        printf("NS: Client '%s' requested operation: %s (path: '%s')\n", 
               req.username, op_to_string(req.op), req.path);
...
void* handle_ss_cmd_session(void* arg) {
    ...
    if (recv(sock, &info_pkt, sizeof(SS_Info_Packet), 0) <= 0) {
    ...
    if (existing_partner != NULL) {
        // Reconnection: restore partnership
        ...
        // The existing_partner (survivor) should send full sync to reconnecting ss
    ...
    if (!info_pkt.is_empty) {
        ...
        while (recv(sock, &sync_pkt, sizeof(SS_File_Sync_Packet), 0) > 0) {
    ...
    if (!ss->backed_up && ss->partner_name[0] == '\0') {
        if (info_pkt.is_empty) {
            // Empty SS - try to pair with unbacked regular SS
            SSInfo* regular_ss = find_unbacked_ss();
            if (regular_ss != NULL) {
                // Pair them
    ...
    // 3. Mark SS as active after successful sync
    ss->is_active = 1;
...
void* handle_ss_update_session(void* arg) {
    ...
    while (1) {
        ...
        int bytes = recv(sock, &opcode, sizeof(SSOpCode), 0);
        ...
        if (opcode == SS_NS_UPDATE_INFO) {
            ...
            recv(sock, info_content, MAX_INFO_LEN, 0);
    ...
    if (ss->partner_name[0] != '\0') {
        ...
        while (partner != NULL) {
            if (strcmp(partner->name, ss->partner_name) == 0 && partner->is_active) {
                ...
                send(partner->cmd_sock, &opcode, sizeof(NSOpCode), 0);
...
void handle_op_create(int sock, ClientRequest* req) {
    ...
    send(target_ss->cmd_sock, &op, sizeof(NSOpCode), 0);
    send(target_ss->cmd_sock, req->path, MAX_PATH_LEN, 0);
    send(target_ss->cmd_sock, req->username, MAX_USERNAME_LEN, 0);
    
    // Receive response from SS
    ServerResponse ss_res;
    recv(target_ss->cmd_sock, &ss_res, sizeof(ServerResponse), 0);
    ...
    if (ss_res.status == ERR_OK) {
        ...
        if (target_ss->is_active) {
            ...
            pthread_mutex_lock(&target_ss->cmd_lock);
            send(target_ss->cmd_sock, &info_op, sizeof(NSOpCode), 0);
            send(target_ss->cmd_sock, req->path, MAX_PATH_LEN, 0);
            
            ServerResponse info_res;
            recv(target_ss->cmd_sock, &info_res, sizeof(ServerResponse), 0);
            if (info_res.status == ERR_OK) {
                recv(target_ss->cmd_sock, info_content, MAX_INFO_LEN, 0);
...
void handle_op_delete(int sock, ClientRequest* req) {
    ...
    for (int i = 0; i < ss_count; i++) {
        if (ss_list[i]->is_active) {
            ...
            send(ss_list[i]->cmd_sock, &op, sizeof(NSOpCode), 0);
            send(ss_list[i]->cmd_sock, req->path, MAX_PATH_LEN, 0);
            
            // Wait for acknowledgment
            ServerResponse ss_res;
            recv(ss_list[i]->cmd_sock, &ss_res, sizeof(ServerResponse), 0);
...
void handle_op_checkpoint(int sock, ClientRequest* req) {
    ...
    send(target_ss->cmd_sock, &op, sizeof(NSOpCode), 0);
    send(target_ss->cmd_sock, req->path, MAX_PATH_LEN, 0);
    send(target_ss->cmd_sock, req->arg1, MAX_PATH_LEN, 0);
    
    // Wait for acknowledgment
    ServerResponse ss_res;
    memset(&ss_res, 0, sizeof(ServerResponse));
    int recv_bytes = recv(target_ss->cmd_sock, &ss_res, sizeof(ServerResponse), 0);
    pthread_mutex_unlock(&target_ss->cmd_lock);
...
void handle_op_exec(int sock, ClientRequest* req) {
    ...
    ss_req.op = OP_READ;
    send(ss_sock, &ss_req, sizeof(ClientRequest), 0);
...
void handle_op_approve(int sock, ClientRequest* req) {
    ...
    if (result == ERR_OK || result == ERR_FILE_EXISTS) {
        ...
        sprintf(res.message, "Access granted to '%s' for '%s' (%c).",
                access_req->username, access_req->path, access_req->type);
    } else {
        res.status = result;
...
void handle_op_createfolder(int sock, ClientRequest* req) {
    ...
    if (ss_res.status == ERR_OK) {
        ...
        while (current != NULL) {
            if (current->is_active && current != target_ss) {
                ...
                send(current->cmd_sock, &repl_op, sizeof(NSOpCode), 0);
                send(current->cmd_sock, req->path, MAX_PATH_LEN, 0);
                
                // Don't wait for response - async replication
                ServerResponse dummy_res;
                recv(current->cmd_sock, &dummy_res, sizeof(ServerResponse), 0);
    ...
    send(sock, &ss_res, sizeof(ServerResponse), 0);
}
...
void handle_op_move(int sock, ClientRequest* req) {
    ...
    for (int i = 0; i < ss_count; i++) {
        ...
        if (ss->is_active) {
            ...
            send(ss->cmd_sock, &op, sizeof(NSOpCode), 0);
            send(ss->cmd_sock, req->path, MAX_PATH_LEN, 0);  // Old path
            send(ss->cmd_sock, new_path, MAX_PATH_LEN, 0);   // New path
            
            ServerResponse ss_res;
            recv(ss->cmd_sock, &ss_res, sizeof(ServerResponse), 0);
